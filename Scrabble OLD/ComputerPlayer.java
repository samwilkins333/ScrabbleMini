...
public class ComputerPlayer implements Playable {
	...
	public void makeMove() {
		int moveInt = _scrabbleGame.getReferee().getMoveInt();
		if (moveInt == 1) {
			_newestWord = this.getBestFirstWord();
		} else if (moveInt > 1) {
			_newestWord = this.getBestWord();
		}
		//system.out.printf("RACK = %s, KERNEL = %s\n", _rack, _kernel);
		if (_newestWord != null) {
...
//			_precedingKernelTiles = new HashMap<String, ArrayList<Tile>>();
//			_succeedingKernelTiles = new HashMap<String, ArrayList<Tile>>();
...
//								if (prefixExtended == true) {
//									validWord.addAdjacentTiles(_precedingKernelTiles.get(_kernel));
//								}
					...
					if (numSuffixSlots > 0 && _suffixes.size() > 0) {
						...
						for (int i = 0; i < _suffixes.size(); i++) {
							...
							if (_scrabbleGame.dictionaryContains(concat) && !_validStrings.contains(concat)) {
								...
								validWord.addKernelTiles(kernelTiles);
//								if (suffixExtended == true) {
//									validWord.addAdjacentTiles(_succeedingKernelTiles.get(_kernel));
//								}
								_validWords.add(validWord);
								_validStrings.add(concat);
							}
					...
					if (numSuffixSlots > 0 && numPrefixSlots > 0 && _specificSuffixes.size() > 0) {
