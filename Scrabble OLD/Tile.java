...
public class Tile {
	...
	private void setUpOverlapFlash() {
		...
		_overlapFlash.setAutoReverse(false);
	...
	public void checkOutOfBoard() {
		Boolean status = true;
		if (this.getCenterX() >= Constants.X0 * Constants.GRID_FACTOR
				&& this.getCenterX() < Constants.X15 * Constants.GRID_FACTOR
				&& this.getCenterY() >= Constants.Y0 * Constants.GRID_FACTOR
				&& this.getCenterY() < Constants.Y15 * Constants.GRID_FACTOR) {
			status = false;
		}
		if (status == true) {
			_tileViewer.setLayoutX(_x * Constants.GRID_FACTOR + Constants.TILE_PADDING);
		...
		EventHandler<MouseEvent> releaseHandler = new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent event) {
				if (Tile.this.isDraggable()) {
					...
					_tilesOnBoard = _thisGame.getTilesOnBoard();
					...
					if (_thisGame.boardSquareOccupiedAt(_xIndex, _yIndex) || event.getClickCount() == 2) {
						_tileViewer.setLayoutX(_x * Constants.GRID_FACTOR + Constants.TILE_PADDING);
						_tileViewer.setLayoutY(_y * Constants.GRID_FACTOR + Constants.TILE_PADDING);
						_checkViewer.setLayoutX(_x * Constants.GRID_FACTOR + Constants.TILE_PADDING);
						_checkViewer.setLayoutY(_y * Constants.GRID_FACTOR + Constants.TILE_PADDING);
	...
	public void refreshPlayerInfo() {
		_currentPlayer = _thisGame.getReferee().getCurrentPlayer();
