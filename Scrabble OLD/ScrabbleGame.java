...
public class ScrabbleGame {
	...
	private ArrayList<String> _validWordsOne;
	private ArrayList<String> _validWordsTwo;
	...
	public ScrabbleGame(PaneOrganizer organizer, Pane root, Pane boardPane) {
		...
		_validWordsOne = new ArrayList<String>();
		_validWordsTwo = new ArrayList<String>();
	...
	public void think(String id) {
		this.clearArrayLists();
		if (id == "PLAYER ONE") {
			this.collectPermutations(this.tilesToString(_playerOneRack), _validWordsOne, 7, "VALID");
		} else if (id == "PLAYER TWO") {
			this.collectPermutations(this.tilesToString(_playerTwoRack), _validWordsTwo, 7, "VALID");
		}
	}
	...
	public void clearArrayLists() {
		_fadeOuts.clear();
		_fadeIns.clear();
		_validWordsOne.clear();
		_validWordsTwo.clear();
	...
	public void setUpDictionary() {
		try {
			_scanner = new Scanner(new File("dictionary.txt"));
		...
		// //system.out.printf("Dictionary has %s terms\n", _dictionary.size());
	}

	public String tilesToString(ArrayList<Tile> playerRack) {
		ArrayList<String> letters = new ArrayList<String>();
		for (int i = 0; i < playerRack.size(); i++) {
			letters.add(playerRack.get(i).getLetter());
		}
		String resultant = "";
		for (int i = 0; i < letters.size(); i++) {
			resultant = resultant + letters.get(i);
		}
		return resultant;
	}
	...
	public int evaluateWord(String word) {
		...
		for (int i = 0; i < word.length(); i++) {
			...
			value = value + Constants.VALUES.get(letter);
		}
		if (word.length() == 7) {
			value = value + 50;
		}
		// //system.out.printf("Word %s has a value of %s\n", word, value);
		return value;
	}

	public String getBestWord(ArrayList<String> validWords) {
		String bestWord = "";
		int bestVal = 0;
		for (int i = 0; i < validWords.size(); i++) {
			String thisWord = validWords.get(i);
			int thisVal = this.evaluateWord(thisWord);
			if (thisVal > bestVal) {
				bestWord = thisWord;
				bestVal = thisVal;
			}
	...
	public void createBestWords() {
		if (_validWordsOne.size() > 0) {
			String oneBest = this.getBestWord(_validWordsOne);
			_playerOneBest = new Word(oneBest, this.evaluateWord(oneBest));
			_playerOneBest.addTiles(this.transferTilesFromRack(oneBest, _playerOneRack));
			// _playerOneBest.printData();
		} else {
			//system.out.println("No valid words can be made from P1's rack");
			if (_referee.getMoveInt() == 1) {
				_referee.nextMove();
			}
		}
		if (_validWordsTwo.size() > 0) {
			String twoBest = this.getBestWord(_validWordsTwo);
			_playerTwoBest = new Word(twoBest, this.evaluateWord(twoBest));
			_playerTwoBest.addTiles(this.transferTilesFromRack(twoBest, _playerTwoRack));
			// _playerTwoBest.printData();
		} else {
			//system.out.println("No valid words can be made from P2's rack");
			if (_referee.getMoveInt() == 1) {
				_referee.nextMove();
			}
		}
...
//	}
	...
	public void displayOutcome(String winner) {
		_endGame = new ArrayList<Tile>();
		for (int i = 0; i < winner.length(); i++) {
			String letter = String.valueOf(winner.charAt(i));
			for (int j = 0; j < _tilesOnBoard.size(); j++) {
				Tile thisTile = _tilesOnBoard.get(j);
				if (thisTile.getLetter().equals(letter)) {
					_endGame.add(thisTile);
					_tilesOnBoard.remove(thisTile);
					break;
				}
			}
		}
		for (int i = 0; i < _tilesOnBoard.size(); i++) {
			_tilesOnBoard.get(i).fadeOut();
		}
		PauseTransition delayPlacement = new PauseTransition(Duration.seconds(4));
		delayPlacement.setOnFinished(new ClearHandler());
		delayPlacement.play();
	}
