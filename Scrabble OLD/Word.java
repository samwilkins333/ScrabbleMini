...
//		_isHumanGenerated = true;
	...
	public Boolean containsAnAddedTile() {
		if (_scrabbleGame.getReferee().getMoveInt() <= 1) {
			return true;
		} else {
			for (int i = 0; i < _tiles.size(); i++) {
				Tile thisTile = _tiles.get(i);
...
//				System.out.printf("%s tile added to board at %s, %s\n", thisTile.getLetter(), x, y);
			...
			int bonus = 0;
			if (_scrabbleGame.getReferee().currentRackIsEmpty()) {
				bonus = 50;
			}
...
//			System.out.println("No word placed - not playable");
	...
	public boolean isFormatted() {
		if (this.isAligned() && this.isCompact() && this.containsAnAddedTile()) {
			return true;
		} 
		return false;
	}
